...
public class BiomeLocationTypeTest {
    ...
    public void canMatch_LocationObject_ReturnsTrue() throws Exception {
        ...
        Object o = new Location<Extent>(e, new Vector3i(1, 0, 1));
    ...
    public void parseMatcher_ValidMapElement_ReturnsCorrectSpongeMatcher() throws SpongeMatcherParseException {
        StringElement element = StringElementParser.parseStringElement("{'x':1,'y':1}");

        SpongeMatcher<Location<?>> matcher = MatcherType.BIOME_LOCATION.parseMatcher(element);

        Extent e = mock(Extent.class);
        assertThat(matcher, matches(new Location<Extent>(e, new Vector3i(1, 0, 1))));
        assertThat(matcher, not(matches(new Location<Extent>(e, new Vector3i(0, 0, 0)))));
    }
}
